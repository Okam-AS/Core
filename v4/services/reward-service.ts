...
export class RewardService {
  ...
  public async GetPurchase(rewardPurchaseId: string): Promise<RewardPurchase> {
    const response = await this._requestService.GetRequest("/rewards/purchase/" + rewardPurchaseId);
    const parsedResponse = this._requestService.TryParseResponse(response);
    if (parsedResponse === undefined) {
      throw new Error("Failed to get reward purchase");
    }
    return parsedResponse;
  }
