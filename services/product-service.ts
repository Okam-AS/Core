...
export class ProductService {
  ...
  public async GetFavoritesWithSearchOptions(storeId: number, searchOptions: { deliveryType?: string } = {}): Promise<Array<Product>> {
    const response = await this._requestService.PostRequest("/products/favorites/" + storeId + "/search", searchOptions);
  ...
  public UploadImage(imagePath: string, productId: string) {
    return this._requestService.FormdataRequest('/products/image', HttpMethod.POST, 'Image', imagePath, [{ name: 'GuidId', value: productId }]);
  }
